...
struct DataAccess {
  DataAccess(const Data & data, int dType)
  ...
  {
    if(dType == REPO)
      return data.repos[object].watches.empty();
    else
      return data.users[object].watches.empty();      
  }

  const Item & object(int object)
  {
    if(dType == REPO)
                 ...
                 DataAccess & access)
{
    ...
    in_cluster.resize(access.capacity(), -1);

    // Random initialization
    for (unsigned i = 0;  i < access.capacity();  ++i) {
    ...
    for (int iter = 0;  iter < 100 && changes != 0;  ++iter) {
	    ...
	    // cluster.fill(0.0);
	      ...
	      const Item &item = access.object(cluster.members[j]);
        ...
        for (unsigned i = 0;  i < access.capacity();  ++i) {
            const Item & object = access.object(i);
            ...
            for (unsigned j = 0;  j < nclusters;  ++j) {
                float score
		  = clusters[j].sim(object,access.ns());
...
void kmeans_repos(Data & data)
{
    ...
    DataAccess repo_access(data,REPO);
...
// #endif
    ...
    for (unsigned i = 0;  i < repo_in_cluster.size();  ++i)
        data.repos[i].cluster = repo_in_cluster[i];
}
...
{
    for (unsigned i = 0;  i < data.repos.size();  ++i) {
        const Item & repo = data.repos[i];
        stream << i << ":" << repo.cluster << "\n";
    }
}
...
{
  Data data;
  data.loadWatches("../train/user_watches.txt",USER);
  data.loadWatches("../train/repo_watched.txt",REPO);  
  vector<int> testList;
  read_test("../train/test.txt",testList);

  cout<<"loading over"<<endl;


  // data.loadSim("/home/shane/workspace/Github/lxvector/data2/knni-iuf.txt.0",REPO);
  // ClusterSuggester rsuggester(data,REPO);
  // rsuggester.loadCluster("../train/repos/kmeans.txt_f");
  // cout<<"loading cluster over"<<endl;
  // cout<<"begin ..."<<endl;
  // rsuggester.suggest("../train/kmeans_repo.txt",testList);


  data.loadSim("/home/shane/workspace/Github/lxvector/data2/knnu-iif.txt.0",USER);
  ClusterSuggester usuggester(data,USER);
  usuggester.loadCluster("../train/users/kmeans.txt_f");
  cout<<"loading cluster over"<<endl;
  cout<<"begin ..."<<endl;
  usuggester.suggest("../train/kmeans_user.txt",testList);

  // kmeans_repos(data);
  // ofstream out_repos("../train/repos/kmeans.txt");
  // save_kmeans_repos(out_repos, data);
  // out_repos.close();

  // kmeans_users(data);
  // ofstream out_users("../train/users/kmeans.txt");
  // save_kmeans_users(out_users, data);
  // out_users.close();
